...
function loadProject() {
  $("#container_main").hide();
$("#greenFlag-button").attr('loading','');
  $(".loading-progress-box").show();
  $("#container_play").show();
  $("#greenFlag").show();
  $("#stopAll").show();
  //$('#switchVK').show();
...
!(function (e) {
  ...
  (s.NotSupportedError = n),
    ...
    "function" == typeof define && define.amd
      ...
      ? (module.exports = s)
      : (e.navigator || (e.navigator = {}),
        e.navigator.mediaDevices || (e.navigator.mediaDevices = {}),
        e.navigator.mediaDevices.getUserMedia ||
          (e.navigator.mediaDevices.getUserMedia = s));
})(this);
...
const runBenchmark = function () {
  ...
  const linear = document.getElementById("loading-progress-linear");
  ...
  storage.webHelper.load = function (...args) {
    ...
    result.then(() => {
      ...
      linear.value= complete / total;
      switch (complete) {
        case 1: {
          progress.textContent += " 嘻嘻，还要加载一下啦~";
          break;
        }
        case 2: {
          progress.textContent += " 下面来点不一样的画面";
          break;
        }
        case 3: {
          progress.textContent += " 我不是美编，做不出漂亮的图";
          break;
        }
        case 4: {
          progress.textContent += " 所以，就不辣你们的眼睛了";
          break;
        }
        case 5: {
          progress.textContent += " 只能和你们在此聊聊天了";
          break;
        }
        case 6: {
          progress.textContent += " 这样也是可以打发时间的";
          break;
        }
        case 7: {
          progress.textContent += " 如果您喜欢，请多多转发";
          break;
        }
        case 8: {
          progress.textContent += " 分享给朋友，快乐你、我、她、他、它、TA";
          break;
        }
        case 9: {
          progress.textContent += " 下面是空白（可能是装B失败）";
          break;
        }
        case 10: {
          progress.textContent += " 此处是空白（作者装B失败）";
          break;
        }
        case 11: {
          progress.textContent += " 假装成真也有点难哦";
          break;
        }
        case 12: {
          progress.textContent += " 装不下去了";
          break;
        }
        case 13: {
          progress.textContent += " 实在装不下去了";
          break;
        }
        case 14: {
          progress.textContent += " 真的装不下去了";
          break;
        }
        case 15: {
          progress.textContent += " 假装有加载动画，最后一条";
          break;
        }
        default: {
          progress.textContent += " 依然在加载";
        }
      }
    });
    return result;
  ...
  vm.on("workspaceUpdate", () => {
    ...
    initialized = true;
    $("#greenFlag-button").removeAttr('loading');
  ...
  vm.runtime.addListener("MONITORS_UPDATE", (monitors) => {
    monitors.forEach((record, id) => {
      if (!monitorStates[id]) {
        ...
        let name =
          record.params.VARIABLE || record.params.LIST || record.opcode;
...
function VK_key_down(key) {
  Scratch.vm.postIOData("keyboard", { key: key, isDown: true });
}
function VK_key_up(key) {
  Scratch.vm.postIOData("keyboard", { key: key, isDown: false });
}
