...
table.on('tool(TableOne)', function (obj) {
    if (obj.event === 'setState0' || obj.event === 'setState1' || obj.event === 'setState2') {
        ...
        AjaxFn('/admin/works/python/setState', {'id': obj.data.id,'s':s }, function (res){
            layer.msg(res['msg']);
            if ('ok' == res['status']){
        ...
        layer.prompt({title:'修改作品标题',formType:0, maxlength: 32, shadeClose:true, value:obj.data.title}, function (v, index) {
            AjaxFn('/admin/works/python/changeTitle', {'id':obj.data.id, 't': v }, function (res){
                layer.msg(res['msg']);
                if ('ok' == res['status']){
                    _table_render.reload();
                    layer.close(index);
        ...
        layer.confirm(`把【${obj.data.title}】复制为默认作品？`,{title:'重要提示',shadeClose:true}, function (index) {
            AjaxFn('/admin/works/python/setDefaultWork', {id: obj.data.id}, function (res){
                layer.msg(res.msg);
                if ('ok' == res.status){
                    layer.close(index);
                }
